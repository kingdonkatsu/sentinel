...
async function captureVisibleTab(sender) {
    try {
        ...
        const dataUrl = typeof senderWindowId === "number"
        ...
        const messageText = error instanceof Error ? error.message : "Failed to capture visible tab";
...
async function runOcrSpike(imageData) {
    try {
        ...
        if (!response?.ok) {
            const detailedError = typeof response?.error === "string" && response.error.trim().length > 0
                ? response.error
        ...
        const messageText = error instanceof Error ? error.message : "ocr spike recognition failed";
        return { ok: false, error: `offscreen_runtime_error: ${messageText}` };
    }
}
function safeSerializeForError(value) {
    try {
...
async function pingOcrSpikeOffscreenDocument() {
    try {
        const response = await withTimeout(chrome.runtime.sendMessage({
        ...
        const messageText = error instanceof Error ? error.message : "ping threw";
        return { ok: false, error: messageText };
    }
}
async function ensureOcrSpikeOffscreenDocument() {
    try {
        ...
        const messageText = error instanceof Error ? error.message : "Failed to create offscreen document";
...
async function hasOcrSpikeOffscreenDocument() {
...
function isOcrSpikeRecognizeMessage(message) {
