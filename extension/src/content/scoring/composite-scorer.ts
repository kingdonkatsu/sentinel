...
import type { ModalityResult, ModalityType } from "../../shared/types";

export const BASE_WEIGHTS: Record<ModalityType, number> = {
  text:     0.35,
  visual:   0.25,
  temporal: 0.20,
  video:    0.15,
...
export interface FusionResult {
  ...
  /** Per-modality effective weights after normalization (for debug/logging) */
...
export class CompositeScorer {
  ...
  fuse(
    ...
    baseWeights: Record<ModalityType, number> = BASE_WEIGHTS
  ): FusionResult {
    ...
    const effectiveWeights: Partial<Record<ModalityType, number>> = {};
    let weightSum = 0;
    ...
    if (weightSum === 0) {
      ...
      for (const key of Object.keys(effectiveWeights) as ModalityType[]) {
    ...
    const contributing = available.filter(
      (r) => (effectiveWeights[r.modality] ?? 0) > 0
    );
    ...
    const hasCriticalSignal = available.some(
      (r) => r.score >= 90 && r.confidence >= 0.8
    );
