export interface VisualContextCues {
  bloodLike: boolean;
  pillLike: boolean;
...
export function extractVisualContextCues(imageData: ImageData): VisualContextCues {
  ...
  const bloodCueScore =
    clamp01(normalize(darkRedRatio, 0.01, 0.09)) * 0.7 +
    clamp01(normalize(deepRedRatio, 0.02, 0.15)) * 0.3;
  const medicalCueScore =
    clamp01(normalize(brightLowSatRatio, 0.15, 0.65)) * 0.65 +
    clamp01(normalize(coolClinicalRatio, 0.05, 0.35)) * 0.35;
  const pillCueScore =
    clamp01(normalize(isolatedBrightDotRatio, 0.0015, 0.02)) * 0.85 +
    clamp01(normalize(brightLowSatRatio, 0.03, 0.28)) * 0.15;
  const injuryCueScore =
    bloodCueScore * 0.55 +
...
export function applyVisualContextCues(
  score: number,
  confidence: number,
...
interface SampledImageStats {
  width: number;
  height: number;
  brightLowSatCount: number;
  coolClinicalCount: number;
  deepRedCount: number;
  darkRedCount: number;
  isolatedBrightDotCount: number;
  edgeDensity: number;
}

function sampleImage(imageData: ImageData, step: number): SampledImageStats {
  ...
  for (let y = 1; y < height - 1; y += 1) {
    for (let x = 1; x < width - 1; x += 1) {
      ...
      if (brightMask[index] === 0) continue;
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      if (x + 1 < width) {
        ...
        if (Math.abs(current - right) >= 28) edgeCount += 1;
        edgeChecks += 1;
      }
      if (y + 1 < height) {
        const below = luminance[index + width] ?? 0;
        if (Math.abs(current - below) >= 28) edgeCount += 1;
        edgeChecks += 1;
      }
    }
  }
